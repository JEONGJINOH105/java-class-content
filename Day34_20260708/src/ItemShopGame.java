...
/*
 * 1. Scanner, ArrayList(가방) 생성 소지금 = 100 ↓ 2. 바깥 while(true) → 메인 메뉴 출력 (소지금 표시
 * + 1.구매 2.가방 3.종료) ↓ 3. 메뉴선택 입력받기 ↓
 * ┌─────────────┬─────────────────┬─────────────┐ │ 메뉴선택==1 │ 메뉴선택==2 │ 메뉴선택==3
 * │ ↓ ↓ ↓ 4-A. 아이템목록 4-B. 가방 출력 4-C. 종료 메시지 출력(서브메뉴) + break ↓ 5-A. 아이템선택번호 입력
 * ↓ 6-A. 0번 → break(서브메뉴 탈출) 1~4번 → 소지금 >= 아이템가격 이면? 예: 소지금 -= 가격,
 * 가방.add(아이템이름) 아니오: "돈이 부족합니다" 출력 ↓ 7-A. 다시 5-A로 돌아가서 반복 ↓ 8. 바깥 while 맨 위로
 * 돌아가서 반복
 */