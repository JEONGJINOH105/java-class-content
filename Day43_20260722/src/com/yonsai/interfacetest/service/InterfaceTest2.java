...
// 카카오페이가 추가된다.
class 카카오결제{
public void 카카오결제하기(int 금액) {
System.out.println("카카오결제 "+ 금액 +"결제");
}
}
// 새 결제 기능은 카카오페이인테, 기존 서비스 주문처리 클래스까지 수정해야된다.
class 애플페이결제{
public void 애플페이결제하기(int 금액) {
System.out.println("애플페이결제 "+ 금액 +"결제");
}
}
class 주문처리{
// 결제 진행이 되면 주문 완료! 실제 음식을 만들어서 배달!
// 새 결제가 하나 추가될 때마다 주문처리클래스가 늘어난다. 계속 수정해야된다.
...
}else if(결제방법.equals("카카오")) {
카카오결제 카카오 = new 카카오결제();
카카오.카카오결제하기(금액);

}else if(결제방법.equals("애플페이")) {
애플페이결제 애플페이 = new 애플페이결제();
애플페이.애플페이결제하기(금액);
}
